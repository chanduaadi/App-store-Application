...
class AppStore extends Component {
  ...
  getFilterdApps = () => {
    ...
    const filletrdAppList = appsList.filter(
      eachAppItem => eachAppItem.category === activeTabId,
    )
    const searchResults = filletrdAppList.filter(eachApp =>
  ...
  render() {
    ...
    const fillteredApps = this.getFilterdApps()
    return (
      <div className="bg-container">
        ...
        <ul className="app-list-container">
          {fillteredApps.map(eachApp => (
            <AppItems key={eachApp.appId} appDetails={eachApp} />
          ))}
        </ul>
      </div>
    )
